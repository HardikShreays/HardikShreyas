...
const useShaderAnimation = (mountRef: React.RefObject<HTMLDivElement>, shaderProps: ShaderProps) => {
...
export const ShaderCanvas: React.FC<ShaderCanvasProps> = (props) => {
  ...
  const shaderProps = {
    hue: props.hue ?? 260,
    speed: props.speed ?? 0.15,
    intensity: props.intensity ?? 0.7,
    complexity: props.complexity ?? 6.0,
    warp: props.warp ?? 0.4,
  };
  
  useShaderAnimation(mountRef, shaderProps);
...
export const ControlSlider: React.FC<ControlSliderProps> = ({ label, value, onChange, min, max, step }) => (
