...
export default function BlurTextAnimation({
    ...
    animationDelay = 0 // Keep validation but logically unused for looping now
}: BlurTextAnimationProps) {
    const ref = useRef(null);
    const isInView = useInView(ref, { amount: 0.2 }); // Trigger when 20% visible
    ...
    // Sync animation state with scroll visibility
    useEffect(() => {
        if (isInView) {
            setIsAnimating(true);
        } else {
